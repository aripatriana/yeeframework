...
import com.yeeframework.automate.script.Actions;
...
public class WorkflowYReader {
	...
	public static class TextCompiler {
		...
		public void compile(String rawText) throws ScriptInvalidException {
			...
			tmp.append(rawText);
			if (rawText.endsWith(";")) {
				scripts.add(convertToEngineFormat(check(tmp.toString())));
		...
		public String convertToEngineFormat(String script) {
	...
	public void setBasicScript(String basicScript, WorkflowEntry workflowEntry) throws ScriptInvalidException {
		...
		if (!workflowEntry.checkKeywords(new ArrayList<String>(Keywords.getKeywords())))
	...
	private void setActionType(String actionType, WorkflowEntry workflowEntry) throws ScriptInvalidException {
		...
		if (!workflowEntry.checkActionTypes(new ArrayList<String>(Actions.getActions())))
